...
**Technology Note**: This project is built entirely on free-tier and open-source technologies — Google Gemini API (free tier) for the agentic AI chatbot, scikit-learn (open-source) for ML anomaly detection, MongoDB Atlas (free M0 cluster) for data storage, Nodemailer for automated budget alert emails via Gmail SMTP, Docker Compose for multi-container orchestration, and Vercel (free tier) for frontend deployment. The live application is accessible at [https://vyayadrishti.vercel.app](https://vyayadrishti.vercel.app). This demonstrates that enterprise-grade FinOps capabilities can be delivered without expensive proprietary tooling, making intelligent cloud cost management accessible to teams of any size.
